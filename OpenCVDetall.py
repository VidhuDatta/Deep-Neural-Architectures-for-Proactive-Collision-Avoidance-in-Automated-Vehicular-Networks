    ...
    for bb in bounding_box_set:
        ...
        if bb.location.distance(vehicle.get_transform().location) < 50:
            ...
            if np.dot(np.array([forward_vec.x, forward_vec.y, forward_vec.z]), np.array([ray.x, ray.y, ray.z])) > 1:
                ...
                verts1 = [v for v in bb.get_world_vertices(carla.Transform())]


                for edge in edges:
                    # Join the vertices into edges
                    p1 = get_image_point(verts1[edge[0]], K, world_2_camera)
                    p2 = get_image_point(verts1[edge[1]],  K, world_2_camera)
