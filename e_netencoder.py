...
Original file is located at
    https://colab.research.google.com/drive/1Do9ty56OJSKbvWn8zQYzRceOB4e0SY-i
"""
...
import tensorflow as tf
from tensorflow.keras.layers import Input, Conv2D, BatchNormalization, Activation, MaxPooling2D, Dropout, concatenate, UpSampling2D, add
...
def enet_encoding_block(inputs, filters, dropout_probability=0.1, max_pooling=True):
    """
    E-Net encoding/downsampling block

    Arguments:
        inputs -- Input tensor
        filters -- Number of filters for the convolutional layers
        dropout_probability -- Probability of dropout to prevent overfitting (default is 0.1)
        max_pooling -- Boolean indicating whether to use max pooling to reduce the spatial dimensions of the output volume (default is True)
    Returns:
    ...
    # max pooling
    if max_pooling:
        P = MaxPooling2D(pool_size=(2, 2), strides=(1, 1), padding='same')(C)  # reduce stride to 1
    else:
        P = C
    ...
    if max_pooling:
        skip_connection = MaxPooling2D(pool_size=(2, 2), strides=(1, 1), padding='same')(S)  # reduce stride to 1
    else:
        skip_connection = S
...
def unet_decoding_block(inputs, skip_connection_input, filters):
    """
    Convolutional decoding/upsampling block for SegNet
    ...
    upsampled = Conv2DTranspose(filters, kernel_size=(2,2), strides=(1,1), padding='same')(skip_connection_input)

    # Concatenate with the inputs tensor
...
def enet_unet_model(input_size, filters, n_classes):
    ...
    C5, S5 = enet_encoding_block(C4, filters * 16, max_pooling=False)

    # Expanding Path (decoding)
    U6 = unet_decoding_block(C5, S4, filters * 8)
    U7 = unet_decoding_block(U6, S3,  filters * 4)
    U8 = unet_decoding_block(U7, S2,  filters = filters * 2)
    U9 = unet_decoding_block(U8, S1,  filters = filters)
